    ...
    return(
      <View style={styles.mascotaContainer}>
        <TouchableOpacity onPress={()=>setExpanded(isExpanded?null:item.id)}>
          ...
          <Text>{item.solicitudes.length} solicitudes</Text>
        </TouchableOpacity>
        ...
        {isExpanded &&
          item.solicitudes.map(solicitud=>(
            ...
            >
              <Text>{solicitud.adoptante.nombre}</Text>
...
const styles = StyleSheet.create({
  ...
  mascotaContainer: { padding: 12, borderWidth: 1, borderColor: "#ccc", marginBottom: 12, borderRadius: 8 },
  mascotaNombre: { fontWeight: "bold", fontSize: 16 },
  solicitudContainer: { padding: 8, marginLeft: 16, borderWidth: 1, borderColor: "#eee", borderRadius: 6, marginTop: 4 },
