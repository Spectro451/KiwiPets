...
export default function FormularioAdoptante({ setRedirect }: FormularioAdoptanteProps) {
  ...
  const [edadBuscada, setEdadBuscada] = useState<Edad>(Edad.ADULTO);
  ...
  const handleSave = async () => {
    ...
    try {
      await updateAdoptante(adoptanteRut!, {
        ...
        telefono,
        direccion,
        cantidad_mascotas: cantidadMascotas, // ← así
  ...
  return (
    <View style={{ flex: 1, padding: 20, backgroundColor: theme.colors.background, justifyContent: "center", alignItems: "center" }}>
      <View style={{
        ...
        <Text style={[styles.label, { color: theme.colors.secondary }]}>Cantidad de mascotas:</Text>
        <TextInput value={cantidadMascotas.toString()} onChangeText={t => setCantidadMascotas(Number(t))} placeholder="Cantidad" keyboardType="number-pad" style={[styles.input, { color: theme.colors.text }]} placeholderTextColor={theme.colors.text} />

        <Text style={[styles.label, { color: theme.colors.secondary }]}>Especie preferida:</Text>
        <Picker selectedValue={especiePreferida} onValueChange={(v) => setEspeciePreferida(v)} style={{ color: theme.colors.text }}>
        ...
        <Picker selectedValue={tipoVivienda} onValueChange={(v) => setTipoVivienda(v)} style={{ color: theme.colors.text }}>
        ...
        <Picker selectedValue={sexo} onValueChange={(v) => setSexo(v)} style={{ color: theme.colors.text }}>
        ...
        <Picker selectedValue={edadBuscada} onValueChange={(v) => setEdadBuscada(v)} style={{ color: theme.colors.text }}>
        ...
        <TextInput value={motivoAdopcion} onChangeText={setMotivoAdopcion} placeholder="Motivo" style={[styles.input, { color: theme.colors.text }]} placeholderTextColor={theme.colors.text} multiline numberOfLines={3} />
